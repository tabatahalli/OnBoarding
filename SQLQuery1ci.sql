...
where OwnerId= '1426'
 select DATEDIFF(DD,2018-12-31,2018-01-01)
/****************************************************************************************************/
select  OP.PropertyId, 
		P.Name, 
		TP.PaymentAmount, 
...
(CASE
When tpf.Id = 1 then tp.PaymentAmount/7 * DATEDIFF(D,tp.StartDate,tp.EndDate)
When tpf.Id =2 then tp.PaymentAmount/14 * DATEDIFF(D, tp.StartDate, tp.EndDate)
else tp.PaymentAmount*12/365 * DATEDIFF(D,tp.StartDate,tp.EndDate) 
